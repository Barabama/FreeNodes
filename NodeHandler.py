...
def get_geos(ips: list[str]) -> list[ResData]:
    ...
    for subs in [ips[i:i + 100] for i in range(0, len(ips), 100)]:
        ...
        # 检查速率限制
        with lock:
            response = make_request("POST", url, params, json.dumps(subs))
...
class VLESSParser:
    ...
    def parse(cls, scheme: str, body: str) -> VLESS:
        ...
        add, port = rest.split(":")
        cls.body = VLESS(id=id, add=add, port=port, query=cls.url.query,
    ...
    @classmethod
    def pack(cls) -> str:
        components = tuple([cls.scheme, cls.url.netloc, cls.url.path,
...
class NodeHandler:
    ...
    def __init__(self, nodes_str: str):
        nodes_str = nodes_str if not is_base64(nodes_str) else \
            b64decode(nodes_str).decode("utf-8")
        self.nodes = []
        self.adds = []
